...
| M2 | `BranchCameraSchema` | `Branch`, `Camera` (task T-12) |
| M3 | `DeviceAndActivationKeySchema` | `Device` (with the filtered unique index on `DeviceId` and the unique constraint on `BranchId`) and `ActivationKey` (task T-13) |
