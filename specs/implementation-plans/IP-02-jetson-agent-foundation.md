...
> **Note (amended by IP-05 — reactivation-security change):** the Backend now, at the moment the Admin *regenerates* the key, immediately revokes the device's current shared secret and sets its status to `ReactivationRequired` (FS-02 §5.3, amended). This is a Backend/Dashboard change only — **the Agent's reactivation flow in this plan is unchanged**: it still makes exactly one `POST /api/v1/activate` call with the new key, retains its `DeviceId`, atomically replaces its local shared secret, preserves `ActivatedAt`, advances `LastActivatedAt`, and never auto-retries. The Agent never observes the revocation directly (it makes no authenticated operational call in this milestone). T-41's MAC-12 reactivation evidence additionally observes the Dashboard showing **Reactivation required** after regeneration and **Activated** again after the Agent reactivates.
