...
> **Note (amended by IP-05 — reactivation-security change):** the Backend now, at the moment the Admin *regenerates* the key, immediately revokes the device's current shared secret and sets its status to `ReactivationRequired` (FS-02 §5.3, amended). **Within IP-02's scope the Agent's reactivation flow is unchanged**: it still makes exactly one `POST /api/v1/activate` call with the new key, retains its `DeviceId`, atomically replaces its local shared secret, preserves `ActivatedAt`, advances `LastActivatedAt`, and never auto-retries. T-41's MAC-12 reactivation evidence observes the Dashboard showing **Reactivation required** after regeneration and **Activated** again after the Agent reactivates.
>
> **Superseded (2026-07-21):** the earlier claim that this change requires "no Agent production change" is withdrawn. IP-05 (Agent-lock expansion) adds Agent production behaviour — a periodic device-**credential-validation** call (detect-only), a persistent local **locked** (`ReactivationRequired`) state, an operational-lock coordinator, and an SQLite schema-v2 migration — so a *running* Agent detects confirmed revocation and stops operational functionality until manually reactivated. That work is owned by IP-05, not IP-02; IP-02's activation foundation is unchanged.
...
| `ActivationService` | First activation persists identity + deletes a file key; already-activated + no key is a no-op; reactivation retains `DeviceId`/`ActivatedAt` and replaces the secret; a returned `DeviceId` mismatch fails loudly; a timeout maps to an ambiguous outcome; a Backend-success/persistence-failure raises non-retryably; `ConfigCache` untouched |
| `ActivationKeyResolver` | Env key wins over the file (file not read); file key stripped of a trailing newline and rejected when empty; missing both → no key; key is a `SecretStr`; only a file key is deleted after success |
| Startup decision (§12.1) | All four branches, driven through the real FastAPI lifespan: no identity + key → activate; no identity + no key → fail startup; identity + key → reactivate; identity + no key → normal restart with no HTTP call; runtime published only on success, owned client closed on failure, ConfigCache loaded (may be empty) and never written |
...
| Runtime | `main.run()` invokes Uvicorn with exactly one worker (ADR-010); `main:app` is the entry point |
...
- **Status: COMPLETE (delivered).** Commit `feat(agent): add validated agent settings`.
  - Added `src/weapon_detection_agent/config/settings.py` (`config/__init__.py` + `settings.py`): an immutable (`frozen=True`) `AgentSettings(BaseSettings)` with `env_prefix="WDA_"`, an explicit `load_settings(**overrides)` loader, and a `ConfigurationError`. Fields: `backend_base_url` (required; validated for a non-blank `http`/`https` URL with a host, whitespace-trimmed, not otherwise normalized — no path appended), `activation_key` (`SecretStr | None`, redacted), `root_path` (`Path`, default `/opt/weapon-detection`, no I/O), `http_timeout_seconds` (`float`, default `10`, must be > 0), `log_level` (default `INFO`, validated against the standard level names). Added `tests/test_settings.py` (20 tests). Added runtime dependency `pydantic-settings>=2.4,<3.0` to `pyproject.toml`.
  - **Precedence:** constructor overrides → `WDA_` environment → declared defaults (pydantic-settings default order). No `.env` loading (out-of-band provisioning, ASM-006). **Fail-fast:** a missing/blank/invalid `WDA_BACKEND_BASE_URL` raises `ConfigurationError` naming the variable; the message is built from field names only, never provided values, so no secret or URL leaks (verified by test). No `os.environ` read outside this module.
  - **Scope note (IP-02 §6.1):** only the **environment** source of the Activation Key is implemented here. The `<root>/config/activation-key` **file** fallback is deferred — it needs the resolved filesystem root (T-34) and reads a file, whereas T-32 settings loading performs **no** filesystem, network, database, or Jetson I/O (asserted by test). The environment half of the env-over-file precedence is in place. The module path is `config/settings.py` per §4 (import path `weapon_detection_agent.config.settings`).
  - Verification on a workstation (Python 3.13, no Jetson/Backend): `pip install -e ".[dev]"` succeeded; `pytest` → 25 passed (20 settings); `ruff check .` → clean; `ruff format --check .` → clean; `mypy src` → no issues; side-effect-free import with the variable unset → `app.title` prints `Weapon Detection Agent`; `load_settings().backend_base_url` with the variable set → `http://localhost:5230`.
  - No logging, filesystem, SQLite, activation, runtime startup, DeepStream, or deployment behaviour was implemented (those remain T-33–T-41). The T-31 minimal `main.py` app is unchanged and its smoke tests still pass. No secret, `.env`, `*.db`, or virtual environment was committed. No Backend/Angular/SRS/ARCH-001/FS-02 file was modified. T-33–T-41 not started; OI-1–OI-4 unchanged.
...
- **Status: COMPLETE (delivered).** Commit `feat(agent): add structured logging and redaction`.
  - Added the `src/weapon_detection_agent/logging/` subpackage: `redaction.py` (central `Redactor` + `is_sensitive_key` + `REDACTED` marker), `formatter.py` (`JsonLogFormatter` — newline-delimited JSON, UTC ISO-8601 timestamps, safe structured exceptions, non-serializable-value coercion), `configuration.py` (`configure_logging(...)` + `LoggingConfigurationError`), and `__init__.py`. Added `tests/test_logging.py` (24 tests). Exposed `VALID_LOG_LEVELS` + `normalize_log_level` from `config/settings.py` so the level validation is shared (IP-02 §10), with the T-32 field validator now delegating to it (behaviour unchanged). Added `*.log`/`logs/` to `agent/.gitignore`.
  - **No new dependency** — standard library only (`logging`, `json`, `datetime`, `traceback`, `re`, `pathlib`). **Structural redaction:** sensitive field names (case/separator-insensitive, with a harmless-metadata allowlist for `secret_status`/`token_count`/`credential_identifier`), registered secret literals, `Bearer <token>` spans, and pydantic `SecretStr` are all scrubbed to `[REDACTED]` across messages, structured `extra` fields, and exception message + traceback — recursively and without mutating the caller's objects. `Redactor.__repr__` never reveals its registered secrets.
  - **Destinations:** stdout always; an optional **explicit** file path with the same records. **Filesystem boundary (IP-02 §9):** the log directory is never created — a missing parent raises `LoggingConfigurationError`; provisioning `<root>/logs/` remains T-34's job. **No rotation/retention** committed (plain `FileHandler`). Nothing is configured at import time; wiring into runtime startup remains T-39.
  - **Scope note:** IP-02 §4's single `logging/setup.py` sketch was realized as a small `logging/` subpackage (`configuration.py`/`formatter.py`/`redaction.py`) for a clear split between wiring, format, and redaction; the entry point is `configure_logging`. Absolute imports ensure the package never shadows the stdlib `logging`.
  - Verification on a workstation (Python 3.13, no Jetson/Backend): `pip install -e ".[dev]"` succeeded; `pytest` → 49 passed (24 logging); `pytest tests/test_logging.py` → 24 passed; `ruff check .` → clean; `ruff format --check .` → clean; `mypy src` → no issues; importing `weapon_detection_agent.main` left root-logger handler count unchanged (0 → 0); the §16 structured smoke emitted valid JSON with the secret absent and `[REDACTED]` present.
  - No filesystem provisioning, SQLite, activation, DeepStream, systemd, heartbeat, or full startup integration was implemented (those remain T-34–T-41). The T-31 minimal `main.py` app is unchanged; T-31/T-32 tests still pass. No secret, `.env`, `*.db`, `*.log`, or virtual environment was committed. No Backend/Angular/SRS/ARCH-001/FS-02 file was modified. T-34–T-41 not started; OI-1–OI-4 unchanged.
...
- **Status: COMPLETE (delivered).** Commit `feat(agent): add filesystem layout provisioning`.
  - Added `src/weapon_detection_agent/config/paths.py`: an immutable (`frozen=True`) `AgentPaths` dataclass resolving the ADR-008 layout from a root — `config_dir`/`database_dir`/`logs_dir`, plus the resolved `database_file` (`database/agent.db`), `activation_key_file` (`config/activation-key`), and `log_file` (`logs/agent.log`) paths — with a `resolve_paths(root)` factory (defaulting to `/opt/weapon-detection`, honouring `WDA_ROOT_PATH` via `AgentSettings.root_path`) and an `AgentPaths.provision()` that creates only this milestone's four directories with their §8 modes (`0750` root/logs, `0700` config/database). Added `tests/test_paths.py` (18 tests, 3 skipped on Windows). **No new dependency** — standard library only (`os`, `dataclasses`, `pathlib`).
  - **Resolution is pure** — building/inspecting an `AgentPaths` performs no filesystem I/O (asserted by test); creation is the separate, explicit `provision()` step. **Provisioning is idempotent and self-healing:** it re-applies each directory's mode on every run (an explicit `os.chmod` after `mkdir`, since `mkdir` is umask-masked and does not alter an existing directory), preserving existing contents (asserted by test). **Directories only, never files** — the database file (T-35), activation-key file (§6.1), and log file (T-33/T-39) are written by the tasks that own them.
  - **Only the four managed directories are created:** the deferred ADR-008 directories `snapshots/`, `recordings/`, `models/`, `pipeline/`, and `runtime/` are asserted absent (no writer yet — Engineering Principle 9). **No ownership change** — the `weapon-detection` service user (D-2) is the installer's job on the Jetson (T-41); this module sets modes only. **Windows (§17):** `modes_enforceable()` gates mode enforcement to POSIX so directory creation still works on Windows while the mode-assertion tests skip; modes are verified for real on Linux/Jetson in T-41.
  - Verification on a workstation (Python 3.13, Windows, no Jetson/Backend): `pytest` → 67 passed, 3 skipped (the POSIX-only mode assertions); `ruff check .` → clean; `ruff format --check .` → clean; `mypy src` → no issues in 9 source files.
  - No SQLite, activation, runtime startup, DeepStream, systemd, or heartbeat behaviour was implemented (those remain T-35–T-41). The T-31 minimal `main.py` app is unchanged; T-31–T-33 tests still pass. No secret, `.env`, `*.db`, `*.log`, or virtual environment was committed. No Backend/Angular/SRS/ARCH-001/FS-02 file was modified. T-35–T-41 not started; OI-1–OI-4 unchanged.
...
- **Status: COMPLETE (delivered).** Commit `feat(agent): add SQLite foundation and schema`.
  - Added the `src/weapon_detection_agent/persistence/` subpackage: `database.py` (connection management — `connect()`, the `open_connection()` closing context manager, the `transaction()` BEGIN/COMMIT/ROLLBACK helper, `DatabaseInitializationError`, and `0600` file-mode enforcement) and `schema.py` (`CURRENT_SCHEMA_VERSION = 1`, the IP-02 §7 version-1 DDL as `SCHEMA_V1_STATEMENTS`, `initialize_database()`/`initialize_schema()`/`read_schema_version()`, and `UnsupportedSchemaVersionError`/`InvalidSchemaStateError`). Added `tests/test_database.py` and `tests/test_schema.py`. **No new dependency** — standard-library `sqlite3` only; no ORM or migration framework (D-6).
  - **Schema (§7 verbatim):** `SchemaVersion(Version INTEGER NOT NULL)`, `DeviceIdentity(SingletonGuard INTEGER PRIMARY KEY CHECK(=1), DeviceId TEXT NOT NULL, ProtectedSharedSecret TEXT NOT NULL, ActivatedAt TEXT NOT NULL, LastActivatedAt TEXT NOT NULL)`, `ConfigCache(SingletonGuard INTEGER PRIMARY KEY CHECK(=1), ConfigJson TEXT NOT NULL, UpdatedAt TEXT NOT NULL)` — verified column-by-column via `PRAGMA table_info`. `SingletonGuard` rejects a second row (primary-key collision) and a non-`1` value (CHECK), both asserted. No `PendingEvents` table; no media/binary storage.
  - **Versioning:** fresh DB → create the three tables + record version 1 atomically; already-current → safe no-op preserving all rows; **newer** version → `UnsupportedSchemaVersionError` (no downgrade/modify); impossible `SchemaVersion` state (0 rows, >1 rows, non-positive) → `InvalidSchemaStateError`. **Transactional:** DDL runs inside one explicit transaction (`isolation_level=None` + `BEGIN`/`COMMIT`/`ROLLBACK`, the mechanism §7's "inside a transaction" requires so DDL rolls back atomically); a forced mid-migration failure leaves **no** tables (rollback asserted). **Idempotent:** no second version row and existing rows survive re-initialization (asserted).
  - **Database location** comes from `AgentPaths.database_file` (T-34); the persistence layer hard-codes no production path and never reads `WDA_ROOT_PATH`. **Filesystem boundary:** the parent `database/` directory is never created here — a missing directory raises `DatabaseInitializationError` (asserted). **PRAGMAs:** `foreign_keys = ON` (per-connection; a no-op for the FK-free v1 tables, enabled for future FK tables); `row_factory = sqlite3.Row`. No WAL/synchronous/busy-timeout/`check_same_thread` set. **File mode:** `0600` on POSIX after creation and reapplied on connect (§7, ARCH-001 §13.3); skipped on Windows (§17); ownership untouched (T-41).
  - **No seeded rows** (`DeviceIdentity`/`ConfigCache` empty after init — asserted). **No repository behaviour** (T-36), activation, Backend contact, startup/FastAPI wiring, or logging configuration. **Import-safe:** importing the persistence modules opens no connection, creates no file, and touches no socket (asserted); there is no module-level connection (asserted). Structural log events (`database_schema_initialization_started`/`_initialized`/`_already_current`) carry only the version, never a row value or secret (asserted, incl. error messages).
  - Verification on a workstation (Python 3.13, Windows, no Jetson/Backend): `pip install -e ".[dev]"` succeeded; `pytest tests/test_database.py tests/test_schema.py` → 31 passed, 3 skipped; full `pytest` → 98 passed, 6 skipped (skips = POSIX-only mode assertions); `ruff check .` → clean; `ruff format --check .` → clean; `mypy src` → no issues in 12 source files; a throwaway-temp-dir smoke test confirmed exactly the three tables, `SchemaVersion = 1`, an idempotent second init, zero seeded rows, only `agent.db` created (no deferred dirs), then removed the temp dir. The T-31 `main.py` app import remains side-effect-free.
  - No SQLite repository, activation, runtime startup, DeepStream, systemd, or heartbeat behaviour was implemented (those remain T-36–T-41). T-31–T-34 tests still pass. No secret, `.env`, `*.db`, `*.log`, or virtual environment was committed. No Backend/Angular/SRS/ARCH-001/FS-02 file was modified. T-36–T-41 not started; OI-1–OI-4 unchanged.
...
- **Status: COMPLETE (delivered).** Commit `feat(agent): add identity and config repositories`.
  - Added to `src/weapon_detection_agent/persistence/`: `models.py` (immutable `DeviceIdentity` and `CachedConfiguration` dataclasses; shared ISO-8601-UTC `to_iso_utc`/`parse_iso_utc` helpers), `errors.py` (`RepositoryError` base + `IdentityAlreadyExistsError`/`InvalidIdentityStateError`/`InvalidConfigCacheStateError`), `device_identity_repository.py` (`DeviceIdentityRepository`), and `config_cache_repository.py` (`ConfigCacheRepository`). Extended `persistence/__init__.py` exports (T-35 exports preserved). Added `tests/test_device_identity_repository.py`, `tests/test_config_cache_repository.py`, `tests/test_repository_integration.py`. **No new dependency** — standard-library `sqlite3`/`dataclasses`/`datetime` plus the existing `pydantic.SecretStr`; no ORM.
  - **DeviceIdentityRepository:** `load()` → stored `DeviceIdentity` or `None` when unactivated; `store(identity)` inserts the singleton in one transaction and **rejects a second store** with `IdentityAlreadyExistsError` (never overwrites — the Device ID is permanent); `replace_shared_secret(shared_secret=…, last_activated_at=…)` atomically updates **only** `ProtectedSharedSecret` + `LastActivatedAt`, **retaining `DeviceId` and `ActivatedAt`** (§10, ADR-015). A forced mid-write COMMIT failure rolls back leaving the prior secret intact (asserted via an injected connection factory); replacing with no identity raises `InvalidIdentityStateError`. The Device-ID **mismatch policy is left to T-38** — no repository operation changes or clears the Device ID.
  - **ConfigCacheRepository:** `load()` → `CachedConfiguration` (raw `ConfigJson` text + UTC `updated_at`) or `None` when absent — the normal state (OI-2). **Load-only:** no writer/replace/clear/refresh (the `ConfigCache` writer is deferred, OI-2). `ConfigJson` is returned raw (no configuration schema exists yet, so it is not parsed) and never logged or placed in an error. "Round-trip when present" (§16.1) is proved by seeding a row with direct SQL, then loading.
  - **Secrets & safety:** the shared secret is a `SecretStr`, absent from `repr`/`str`, logs, and exception messages (all asserted). Stored as its plain value in the `0600` database (D-4) — **no application-layer encryption is added or claimed**. Timestamps are timezone-aware UTC, serialized ISO-8601 UTC, with malformed stored timestamps rejected safely and naive datetimes rejected on store. Structural log events only (`device_identity_loaded`/`_saved`/`_secret_replaced`, `config_cache_loaded`/`_missing`) — the safe `DeviceId` may be logged (§15); no secret or configuration content is.
  - **Construction & boundaries:** each repository takes the explicit `AgentPaths.database_file` (or an injectable `connection_factory`); it resolves no `WDA_ROOT_PATH`, creates no directory, initializes no schema, retains no global connection, and performs no import-time I/O (all asserted). A fresh short-lived connection per operation, closed predictably; the T-35 `transaction()` helper wraps writes. No Backend contact, activation, reactivation, or startup wiring (T-37/T-38/T-39).
  - Verification on a workstation (Python 3.13, Windows, no Jetson/Backend): `pip install -e ".[dev]"` succeeded; `pytest tests/test_device_identity_repository.py tests/test_config_cache_repository.py tests/test_repository_integration.py` → 40 passed; full `pytest` → **138 passed, 6 skipped** (skips = POSIX-only `0600`/mode assertions on Windows, §17); `ruff check .` → clean; `ruff format --check .` → clean; `mypy src` → no issues in 16 source files; a throwaway-temp-dir smoke test confirmed store/load, load-only config, atomic secret replacement retaining `DeviceId`/`ActivatedAt` and advancing `LastActivatedAt`, ConfigCache unchanged, one row per singleton, then removed the temp dir. The T-31 `main.py` app import remains side-effect-free.
...
- Documentation: `agent/README.md` — the activation-service section.
...
- **Status: COMPLETE (delivered).** Commit `feat(agent): add activation orchestration service`.
  - Added `src/weapon_detection_agent/activation/service.py` (`ActivationService`, `ActivationServiceResult`, `ActivationOutcome`) and `src/weapon_detection_agent/activation/key_resolver.py` (`ActivationKeyResolver`, `ResolvedActivationKey`, `KeySource`); extended `activation/errors.py` with the service errors (`ActivationServiceError` base + `ActivationKeyMissingError`/`ActivationKeyFileError`/`DeviceIdentityMismatchError`/`ActivationOutcomeAmbiguousError`/`ActivationPersistenceError`/`ActivationKeyCleanupError`); extended `activation/__init__.py` exports. Added `tests/test_activation_key_resolver.py`, `tests/test_activation_service.py`, `tests/test_activation_service_integration.py`. **No dependency change** — existing httpx client, `pydantic.SecretStr`, stdlib `datetime`/`pathlib`; no retry/DI/ORM library.
  - **Scope note (naming, within §4/§16's delegated space):** the service module is `activation/service.py` (not `activation_service.py`) and the key resolution is split into `activation/key_resolver.py`; both sit inside §4's `activation/` package. The `ActivationService`/`ActivationKeyResolver` names are used consistently across code, tests, and docs.
  - **Key precedence (§6.1, D-1):** `ActivationKeyResolver` returns the `WDA_ACTIVATION_KEY` env value first (passed in as `settings.activation_key`, a `SecretStr` — the file is **not** read), else the `<root>/config/activation-key` file (`AgentPaths.activation_key_file`), else `None`. The resolver never reads `os.environ`. File rule: surrounding whitespace (incl. a trailing newline) is stripped and an empty/whitespace-only file is rejected (`ActivationKeyFileError`); the key is a `SecretStr`; the source (`environment`/`file`) is tracked so only a file key is deleted after success.
  - **First activation (§9.1):** no stored identity + a key → one Backend call → build `DeviceIdentity` (returned `deviceId`/`sharedSecret`, one injected UTC clock value for both `ActivatedAt` and `LastActivatedAt`) → `store()` → delete the key file **iff** file-sourced. No stored identity + no key → `ActivationKeyMissingError` (no Backend call, no writes). `branchId` is returned only in the service result, never persisted (the schema has no branch column).
  - **Already activated / no key (§12.2):** stored identity + no key → no-op — no Backend call, no SQLite change, timestamps untouched, `ActivationOutcome.ALREADY_ACTIVATED` returned. This is the ordinary restart path.
  - **Reactivation (§9.2):** stored identity + a key → one Backend call → compare returned `deviceId` with the stored one → **mismatch → `DeviceIdentityMismatchError`, no update, no file deletion** (OI-4) → else `replace_shared_secret()` (retains `DeviceId` and the original `ActivatedAt`, advances `LastActivatedAt` to the clock value, replaces the secret) → delete a file key after success.
  - **No retry / ambiguous timeout (§14, amended):** exactly one Backend request per call; a timeout → `ActivationOutcomeAmbiguousError` (the Backend may have committed activation; local state unchanged; no retry; operator/new key required); `401`/transport/`5xx`/malformed each propagate the T-37 client's typed error unchanged, with no persistence and no file deletion. **Backend-success-then-persistence-failure** → `ActivationPersistenceError` (no second Backend call, no file deletion, prior local state preserved by the repo transaction — asserted with an injected commit-failing connection). **Cleanup failure after committed persistence** → `ActivationKeyCleanupError`, without rolling back the identity.
  - **Safety & boundaries:** the shared secret and key are `SecretStr`; the service result carries **no** secret; no key/secret/config content appears in any log or error (asserted). Clock is injected (default `datetime.now(timezone.utc)`), validated tz-aware. Dependencies are injected — the service constructs no client/repository/clock globally, reads no settings/env, initializes no schema, provisions no directory, and **writes no `ConfigCache`** (asserted). Importing the service/resolver performs no I/O; `main.py` is unchanged and its import stays side-effect-free (asserted) — **the Agent does not activate at startup; wiring is T-39**.
  - Verification (Python 3.13, Windows, no Backend/network): `pip install -e ".[dev]"` succeeded; `pytest tests/test_activation_key_resolver.py tests/test_activation_service.py tests/test_activation_service_integration.py` → 44 passed; full `pytest` → **243 passed, 6 skipped** (skips = POSIX-only mode assertions, §17); `ruff check .` → clean; `ruff format --check .` → clean; `mypy src` → no issues in 22 source files; a throwaway-temp-dir smoke test drove first activation (identity stored, file removed), a no-key restart (0 Backend calls), and reactivation (DeviceId + ActivatedAt preserved, LastActivatedAt + secret changed, ConfigCache unchanged, one identity row). No secret printed.
  - No FastAPI lifespan/startup wiring, DeepStream, heartbeat, config polling, `ConfigCache` write, alert/event/command handling, or Backend change was implemented (those remain T-39–T-41). `main.py` was not wired to run activation. T-31–T-37 tests still pass. No secret, key file, `.env`, `*.db`, `*.log`, or virtual environment was committed. No Backend/Angular/SRS/ARCH-001/FS-02 file was modified. `ConfigCache` remains **load-only** (OI-2); the activation **no-retry** decision is preserved. T-39–T-41 not started; OI-1–OI-4 unchanged.
...
- **Status: COMPLETE (delivered).** Commit `feat(agent): add startup workflow and runtime`.
  - Added `src/weapon_detection_agent/app.py` (`create_app()` application factory), `src/weapon_detection_agent/runtime/startup.py` (`create_lifespan`, the `_start`/`_shutdown` orchestration, `default_backend_client_factory`, `default_clock`), and `src/weapon_detection_agent/runtime/state.py` (`AgentRuntime`, `get_runtime`, `RUNTIME_STATE_ATTR`). Refactored `main.py` to `app = create_app()` plus a `run()` that pins `workers=1`. Added `tests/test_runtime_state.py`, `tests/test_app_lifespan.py`, `tests/test_runtime_startup.py`. **No new dependency** — FastAPI lifespan + the existing httpx runtime dependency.
  - **Application factory / lifespan:** `create_app(*, settings_loader=load_settings, clock=default_clock, backend_client_factory=default_backend_client_factory)` builds a fresh `FastAPI` with the T-39 lifespan attached; `main:app` remains the Uvicorn entry point. Construction runs **no** startup work; **importing `main` performs no I/O and configures no logging handlers** (asserted, 0→0). Startup runs in the FastAPI **lifespan** (§5), not at import.
  - **Startup order (§12.1):** load settings → resolve paths → provision layout → configure logging (after the layout exists, so the log file's directory is present; stdout + `<root>/logs/agent.log`, with the env Activation Key registered as a redacted literal) → initialize SQLite schema → construct `DeviceIdentityRepository` + `ConfigCacheRepository` → construct the owned `BackendActivationClient` → construct `ActivationKeyResolver` (env `settings.activation_key` + `AgentPaths.activation_key_file`) + `ActivationService` → run `activate()` once → **load ConfigCache** (may be empty, OI-2 — logged, nothing consumed/written) → publish `app.state.runtime` → serve.
  - **Four branches (§12.1):** first activation (persist identity, remove a file key), reactivation (retain `DeviceId`/`ActivatedAt`, advance `LastActivatedAt`, replace secret, remove a file key), already-activated/no-key **no-op** (no Backend call, offline-start), and unactivated+no-key → **fail startup** (`ActivationKeyMissingError`). All asserted via the FastAPI `TestClient` lifespan with a fake Backend client — no real Backend, no network port.
  - **Failure handling:** any activation error (`401`, timeout→ambiguous, transport, `5xx`, malformed, Device-ID mismatch, persistence failure, key-cleanup failure) **fails startup**, closes the owned client, and does **not** publish `app.state.runtime` — asserted, including that the fake client's `activate()` was called **exactly once** (no retry) and `aclose()` ran on failure. Provisioning and schema failures fail before the client is created and propagate their own safe errors. `main.py` is **not** wired to run activation.
  - **Runtime state:** `AgentRuntime` (settings, paths, both repositories, activation result) published on `app.state.runtime` only after full success; `get_runtime(app)` returns `None` before startup / after shutdown. No secret is stored or representable — the key lives only in the settings' `SecretStr`, and the result carries no shared secret (asserted no key/secret in `repr`).
  - **Shutdown (§12.4):** closes the owned Backend client, sets the runtime reference to `None`; idempotent and non-raising; makes **no** Backend call and deletes **no** identity/ConfigCache/database/directory/key file (asserted). A caller-owned injected httpx is **not** closed (T-37 contract, asserted).
  - **Single worker (ADR-010):** the documented command is `uvicorn weapon_detection_agent.main:app --host 0.0.0.0 --port 8000 --workers 1`; `main.run()` pins `workers=1` (asserted by monkeypatching `uvicorn.run`). No CLI-internals inference; systemd enforcement is T-41. **No health/operational endpoint** was added (OI-3) — only FastAPI's built-in `/docs`/`/openapi.json` remain (asserted no `/health`, `/status`, `/ready`, `/live`, `/activate`).
  - Verification (Python 3.13, Windows, no Backend/network): `pip install -e ".[dev]"` succeeded; `pytest tests/test_runtime_startup.py tests/test_app_lifespan.py tests/test_runtime_state.py` → 44 passed; full `pytest` → **287 passed, 6 skipped** (skips = POSIX-only mode assertions, §17); `ruff check .` → clean; `ruff format --check .` → clean; `mypy src` → no issues in 26 source files; the side-effect-free import assertion (0→0 root handlers) passed; a temporary-root lifespan smoke test drove first activation (identity stored, file key removed, client closed on exit, no health route) and a second no-key startup (no-op, 0 Backend calls). No key or secret printed.
  - No DeepStream, detection, alerts, heartbeat, health monitoring, command handling, WebRTC, config polling, or **ConfigCache write** was implemented (those remain excluded / T-40–T-41). T-31–T-38 tests still pass. No secret, key file, `.env`, `*.db`, `*.log`, or virtual environment was committed. No Backend/Angular/SRS/ARCH-001/FS-02 file was modified. `ConfigCache` remains **load-only** (OI-2); the activation **no-retry** decision is preserved; **no Agent health endpoint** exists (OI-3). T-40–T-41 not started; OI-1–OI-4 unchanged.
...
- **Status: COMPLETE (delivered).** Commit `test(agent): verify backend activation contract`.
  - **Layer A — simulated Backend (§16.2), `agent/tests/integration/test_agent_activation_lifespan.py`: 12 tests, all passing.** A stateful in-memory Backend serves the exact §11 contract through `httpx.MockTransport`; every Agent component is real (real `BackendActivationClient`, `ActivationKeyResolver`, `ActivationService`, T-35/T-36 SQLite store, T-39 lifespan) against a temporary root. Covers first activation (identity persisted, file key removed), restart no-op, reactivation, uniform `401`, consumed key on a second Agent, Device-ID mismatch, the four failure modes (timeout→ambiguous, transport, `5xx`, malformed) each raising its typed error with **exactly one** request, runtime published only on success with the owned client closed, `ConfigCache` untouched, and no key/secret in any log.
  - **Layer B — real Backend contract (§16.3), `agent/tests/contract/test_backend_activation_contract.py` + `agent/tests/support/backend_process.py` + `agent/tests/contract/conftest.py`: 10 tests, all passing against the real Backend.** Marked `backend_contract`, deselected from the default run (`addopts = -m 'not backend_contract'`), gated on `WDA_RUN_BACKEND_CONTRACT_TESTS=1`.
  - **Harness design.** The Backend's own `SqlServerApiHostFactory` is an in-process `WebApplicationFactory`/`TestServer` and exposes no TCP socket, so a Python `httpx` client cannot reach it — proving the *wire* contract requires a real port. The harness therefore builds the solution once and launches the built `WeaponDetection.Api.dll` directly (not `dotnet run`, whose child can be orphaned on Windows) bound to `http://127.0.0.1:0`, so Kestrel picks a free loopback port race-free; **never bound publicly**, all configuration through the process environment, never argv. Readiness uses the Backend's **existing** anonymous `GET /api/v1/health` under a bounded timeout — **no health endpoint was added to either side** (OI-3 preserved).
  - **Database isolation.** A uniquely named `WeaponDetectionContract_<random>` database per run, created via `sqlcmd` with **trusted (integrated) authentication** — no SQL password exists to hard-code, print, or commit, and no user-secret is read or modified. The Backend does not migrate at startup, so the real EF migrations are applied explicitly with `dotnet ef database update` through the Backend's existing design-time seam `EFCORE_DESIGNTIME_CONNECTION` — **no Backend production change was required**. Cleanup forces `SINGLE_USER WITH ROLLBACK IMMEDIATE` and drops **only** the generated database; the harness refuses any name outside the `WeaponDetectionContract_` prefix, so a development database cannot be dropped by mistake. Environment used: local SQL Server Express instance (`localhost\SQLEXPRESS` by default, overridable via `WDA_BACKEND_CONTRACT_SQL_SERVER`), Windows integrated authentication — no credential recorded here or in the repository.
  - **Provisioning.** The Branch, Camera, Device, and one-time Activation Key are created through the **real authenticated API** (`POST /api/v1/auth/login`, `POST /api/v1/branches`), the same routes the Dashboard uses — no test-only endpoint, no seeding shortcut, no domain bypass (FS-02 §1.2, §11).
  - **Real results.** First activation: exactly **one** request; the real success envelope parsed; the Backend's public `DeviceId` persisted to SQLite and matching the Dashboard's device view; the issued shared secret persisted intact (asserted as a boolean, never displayed); the key file removed only **after** local persistence; Device flipped to `Activated`; runtime published; `ConfigCache` empty; owned client closed on exit. Consumed key → exact `401`/`INVALID_ACTIVATION_KEY`/`"The activation key is invalid."` with `data` omitted. Invalid key → same uniform `401`, no identity row, key file retained, runtime not published, one request, client still closed. Restart with no new key → `ALREADY_ACTIVATED`, **zero** activation requests (observed via a call counter on the real client, not assumed), identity and `ConfigCache` unchanged. **Real reactivation** through the real regeneration endpoint (`POST /api/v1/devices/{branchId}/activation-key/regenerate`) → one request, **same** public `DeviceId` (no contract contradiction; T-38's mismatch guard untouched), `ActivatedAt` unchanged, `LastActivatedAt` advanced, secret rotated, regenerated key consumed and its file removed; the superseded key then gets the uniform `401`.
  - **Contract drift protection.** A single cross-language assertion exercises the real Agent client against the real Backend and pins method, route, the sole `activationKey` request property, the absence of an `Authorization` header (anonymous endpoint), the success envelope's `data.deviceId`/`data.sharedSecret`/`data.branchId`, the `200`/`401` statuses, and the `INVALID_ACTIVATION_KEY` code. No code generation, no duplicated C# schema, no shared C#/Python runtime library.
  - **Secret safety (MAC-9).** Activation Key and shared secret live in memory only (plus the Agent's protected key file inside a temporary root deleted on teardown); never printed, never in an assertion message, test name, filename, command line, environment variable, or fixture. Secret equality is asserted as a boolean. Backend log tails surfaced in failures pass through a sanitizer scrubbing both values and anything resembling a SQL password.
  - **Cleanup, verified after the run.** No `WeaponDetection.Api` process remained (remaining `dotnet.exe` processes were VS Code C# DevKit and persistent MSBuild build nodes); no `WeaponDetectionContract%` database remained (`sys.databases` query returned empty); no temporary Agent root or key file remained; no `agent.db` or stray `*.db` exists in the repository (only gitignored mypy-cache files); no secret appeared in captured output.
  - **Production code changed: none.** No Backend production file and no Agent production file was modified — the harness needed no seam beyond what already existed (`EFCORE_DESIGNTIME_CONNECTION`, `GET /api/v1/health`, the T-39 `backend_client_factory` injection point). The only non-test change is `agent/pyproject.toml` registering the `backend_contract` marker, adding `tests` to `pythonpath`, and deselecting the marker by default.
  - **Verification.** Agent: Layer A `pytest tests/integration/` → 12 passed; Layer B `WDA_RUN_BACKEND_CONTRACT_TESTS=1 pytest -m backend_contract` → **10 passed** against the real Backend and real SQL Server; full fast suite `pytest` → **299 passed, 6 skipped, 10 deselected** (skips = POSIX-only mode assertions, §17; deselected = the contract suite); `ruff check .` clean; `ruff format --check .` → 48 files already formatted; `mypy src` → no issues in 26 source files. Backend: `dotnet build` → 0 warnings, 0 errors; `dotnet test` → **246 unit + 248 integration = 494 passed, 0 failed**; `dotnet list package --vulnerable --include-transitive` → no vulnerable packages in any of the six projects; `dotnet ef migrations has-pending-model-changes` → "No changes have been made to the model since the last migration."
  - No DeepStream, detection, alerts, snapshots, recordings, heartbeat, health monitoring, command handling, siren, WebRTC, config polling, **ConfigCache write**, retry logic, systemd, or UI work was implemented. No Backend, Angular, SRS, ARCH-001, or FS-02 file was modified. `ConfigCache` remains **load-only** (OI-2); activation **no-retry** is preserved and re-verified on every real path; **no Agent health endpoint** exists (OI-3). T-41 not started; OI-1–OI-4 unchanged.
...
*IP-02 — Jetson Agent Foundation. Status: Draft, awaiting full approval. T-31 (project scaffolding and tooling), T-32 (configuration model and settings loading), T-33 (logging foundation with secret redaction), T-34 (filesystem layout provisioning), T-35 (SQLite foundation and schema), T-36 (device-identity and config-cache repositories), T-37 (Backend activation client), T-38 (activation and reactivation service), T-39 (startup workflow and FastAPI runtime), and T-40 (simulated-Backend integration suite and real-Backend contract suite) have been delivered under explicit task-level approval; T-41 has not been started. Open items OI-1–OI-4 remain open (ConfigCache is load-only under OI-2; no Agent health endpoint exists, OI-3). §14/D-5 amended (approved during T-37): automatic retries for the one-time activation endpoint are prohibited until the Backend supports idempotency.*
