...
*IP-02 — Jetson Agent Foundation. Status: Draft, awaiting full approval. T-31 (project scaffolding and tooling), T-32 (configuration model and settings loading), T-33 (logging foundation with secret redaction), T-34 (filesystem layout provisioning), T-35 (SQLite foundation and schema), T-36 (device-identity and config-cache repositories), T-37 (Backend activation client), and T-38 (activation and reactivation service) have been delivered under explicit task-level approval; T-39–T-41 have not been started. Open items OI-1–OI-4 remain open (ConfigCache is load-only under OI-2). §14/D-5 amended (approved during T-37): automatic retries for the one-time activation endpoint are prohibited until the Backend supports idempotency.*
