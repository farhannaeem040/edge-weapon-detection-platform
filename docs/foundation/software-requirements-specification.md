...
| FR-BRN-005 | The system shall allow the Admin to regenerate/reset a branch's Activation Key, invalidating the previous key. For an already activated device, regeneration shall additionally, in one atomic operation, immediately revoke the device's current shared secret and place the device in a `ReactivationRequired` state — preserving the persistent Device ID and all relationships — so that the device cannot authenticate again until it reactivates with the new key. *(Amended 2026-07-21 — reactivation-security change (IP-05). Superseded behaviour: regeneration previously left the device's shared secret valid and status `Activated` until the Agent later reactivated. Reason: immediate, security-first credential revocation. See FS-02 §5.3.)* |
...
| FR-BRN-008 | The system shall provide a device-authenticated credential-validation endpoint (`POST /api/v1/device/credentials/validate`) by which an activated Jetson Agent can confirm whether its current Device ID and shared secret remain valid. Only a `401 INVALID_DEVICE_CREDENTIALS` from this endpoint is a confirmed revocation; on it the running Agent shall **clear its local shared secret**, enter a persistent locked (`ReactivationRequired`) state, stop operational functionality, retain its Device ID, and resume only after being manually reactivated with a newly provisioned Activation Key. Ambiguous/infrastructural outcomes (`403`, `404`, `5xx`, timeout, transport, malformed) shall not lock the Agent. The endpoint shall never return or distribute an Activation Key or shared secret, shall not act as a heartbeat, and shall use a uniform rejection response. **Offline limitation:** if the Jetson is disconnected when the key is regenerated, the Backend revokes immediately but the Agent detects the revocation only after connectivity returns and its next validation receives the confirmed `401`. *(Added 2026-07-21 — reactivation-security Agent-lock; reason: a running Agent must detect confirmed credential revocation. See FS-02 §8.1/§10.5 and IP-05.)* |

**Acceptance Criteria**: A branch created via the dashboard produces a usable Activation Key; a Jetson Agent using a valid, unconsumed key receives configuration and activates successfully; reuse of a consumed key is rejected; regenerating a key invalidates the old one and a subsequent activation attempt with the old key fails; and regenerating the key of an already-activated device atomically revokes its current shared secret and moves it to `ReactivationRequired` (Device ID retained) until it reactivates with the new key.
...
| NFR-SEC-005 | Security | Device credential validation (FR-BRN-008) shall use a uniform failure response (`INVALID_DEVICE_CREDENTIALS`, `401`) that does not disclose which check failed, and shall never return or distribute credential material. A server-unreachable or ambiguous outcome (timeout, DNS, connection failure, `5xx`, malformed) shall **not** be treated as revocation and shall not lock the Agent (consistent with NFR-REL-001: network failure alone must not stop local operation; only confirmed revocation must). *(Added 2026-07-21 — reactivation-security Agent-lock; see FS-02 §8.1 and IP-05.)* |
| NFR-SEC-002 | Security | The Jetson Agent shall use a one-time Activation Key for initial device activation. Upon successful activation, the Backend shall issue a persistent Device ID and shared secret, which shall authenticate subsequent Agent-to-Backend operational requests. When the Admin regenerates an already-activated device's Activation Key, the Backend shall immediately and atomically revoke the current shared secret so it can never authenticate again, requiring reactivation with the new key before the device is usable. *(Corrected — approved requirements-baseline alignment resulting from Architecture Decision 2 (amended); see Section 9 Traceability Matrix. Amended 2026-07-21 — reactivation-security change (IP-05); superseded behaviour: the secret was previously invalidated only when a new one was issued at the Agent's reactivation; reason: immediate revocation on regeneration. See FS-02 §5.3.)* |
